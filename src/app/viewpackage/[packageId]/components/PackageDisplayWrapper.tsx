...
const PackageDisplayWrapper = ({
  ...
  const handlePrev = () => {
    if (imageIndex !== 0) setImageIndex((x) => x - 1);
  };
  return (
    <div className="mb-2 flex gap-8">
      <div className="flex flex-col gap-2 w-[50rem]">
        <div
          style={{
            backgroundImage: `url(${currentImageArray[imageIndex]})`,
          }}
          className="w-[50rem] transition-all bg-center duration-500 bg-cover
           h-[30rem] rounded-2xl overflow-hidden relative"
        >
          <div className="absolute bg-color9 p-4 top-8 left-8 rounded-lg">
            <Icon.BsArrowLeft />
          </div>
          ...
          <div className="absolute left-8 bottom-8 flex gap-2 items-center">
            {currentImageArray.map((img, index) => (
              <div
                ...
                className="bg-color9  rounded-full "
              ></div>
            ))}
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2 h-[10rem]">
          <div
            onClick={() => {
              setCurrentImageArray(packageData.tourImages);
              setImageIndex(0);
            }}
            style={{
              backgroundImage: `url(${packageData.tourImages[0]})`,
            }}
            className="bg-cover relative overflow-hidden rounded-xl flex items-center text-color9 justify-center text-sm font-semibold py-2 border-4 border-color5"
          >
            <h3 className="relative z-10">TOUR IMAGES</h3>
            <div className="h-full w-full bg-color1/30  absolute"></div>
          </div>
          <div
            onClick={() => {
              setCurrentImageArray(packageData.hotelImages);
              setImageIndex(0);
            }}
            style={{
              backgroundImage: `url(${packageData.hotelImages[3]})`,
            }}
            className="bg-cover relative overflow-hidden rounded-xl flex items-center text-color9 justify-center text-sm font-semibold py-2"
          >
            <h3 className="relative z-10">HOTEL IMAGES</h3>
            <div className="h-full w-full bg-color1/30  absolute"></div>
          </div>
          <div
            onClick={() => {
            ...
            style={{
              backgroundImage: `url(${packageData.hotelImages[1]})`,
            }}
            className="bg-cover relative overflow-hidden rounded-xl flex items-center text-color9 justify-center text-sm font-semibold py-2"
          >
            <h3 className="relative z-10">DESTINATION IMAGES</h3>
            <div className="h-full w-full bg-color1/30  absolute"></div>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="bordes border-color2/20 text-color1 mt-4 p-4">
          <h1 className="font-semibold flex items-center gap-2 text-color1">
            <Icon1.BiPackage /> Package Includes
          </h1>
          ...
          <div className="mt-6">
            ...
            <h3 className="text-2xl font-semibold text-[#ff2a2a]">
          ...
          <div className="flex gap-8">
            <div className="mt-4">
              ...
              <div className="flex">
                <button className="py-3 px-4 text-sm font-semibold border border-color1/50">
                  -
                </button>
                <div className="p-3 text-sm font-semibold border-t border-b border-color1/50">
                  2
                </div>
                <button className="py-3 px-4 text-sm font-semibold border border-color1/50">
                  +
          ...
          <div className="flex gap-12">
            ...
            <div className="text-sm mt-8 font-bold px-3 py-4 justify-center items-center border-2 border-color1 flex w-[10rem]">
              BOOK NOW
            </div>
          </div>
        </div>
