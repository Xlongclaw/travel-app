...
const PackageWrapper = ({ tourPackage }: { tourPackage: packageType }) => {
  const finalPrice = useDiscountPrice(tourPackage.price, tourPackage.discount);
  return (
    <Link
      className="w-[24.3%] flex-shrink-0"
      href={"/viewpackage/" + tourPackage._id}
    >
      <motion.div
        whileHover={{ scale: 1.05 }}
        className=" p-3 pb-6 w-full shadow2 bg-color9 cursor-pointer "
      >
        <Image
          ...
          height={300}
        />
        <div className="text-color1 font-bold my-2">{tourPackage.name}</div>
